...
library(knitr)
tb<-data.frame("Package"=c("EML","emld", "RNeXML", "datapack", "rbefdata", "geometa", "MetaEgress", "EMLassemblyline", "MetaInbase", "LivingNorwayR"), "Package_description"=c(packageDescription(pkg = "EML")$Description, packageDescription(pkg = "emld")$Description, packageDescription(pkg = "RNeXML")$Description,packageDescription(pkg = "datapack")$Description,packageDescription(pkg = "rbefdata")$Description,packageDescription(pkg = "geometa")$Description, packageDescription(pkg = "MetaEgress")$Description, packageDescription(pkg = "EMLassemblyline")$Description, packageDescription("MetaInbase")$Description, packageDescription("LivingNorwayR")$Description))
...
*MetaInbase* [@MetaInbase] has functions to convert EML to tables. It appears to be in an early stage of development
...
#length(report1$DependencyReporter$nodes$node)#49
#length(report2$DependencyReporter$nodes$node)#13
##length(report3$DependencyReporter$nodes$node)#79
#length(report4$DependencyReporter$nodes$node)#48
# length(report5$DependencyReporter$nodes$node)#35
# length(report6$DependencyReporter$nodes$node)#17
# length(report7$DependencyReporter$nodes$node)#16
# length(report8$DependencyReporter$nodes$node)#65
# length(report9$DependencyReporter$nodes$node)#145
# #length(report10$DependencyReporter$nodes$node)#No Dependency
# #length(report11$DependencyReporter$nodes$node)#170

PkgName=c(rep("EML",length(report1$DependencyReporter$nodes$node)), rep("emld",length(report2$DependencyReporter$nodes$node)), rep("RNeXML", length(report4$DependencyReporter$nodes$node)),
          rep("datapack", length(report5$DependencyReporter$nodes$node)), rep("rbefdata", length(report6$DependencyReporter$nodes$node)), rep("geometa", length(report7$DependencyReporter$nodes$node)), rep("MetaEgress",length(report8$DependencyReporter$nodes$node)),
