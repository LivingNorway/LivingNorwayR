# ------ 1. PREPARATION TERM INFORMATION ------
...
getGBIFPreparationTerm <- function() {
	outValue <- NULL
	if("http://data.ggbn.org/schemas/ggbn/terms/Preparation" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://data.ggbn.org/schemas/ggbn/terms/Preparation"]]$termInfo
	} else if("http://data.ggbn.org/schemas/ggbn/terms/Preparation" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://data.ggbn.org/schemas/ggbn/terms/Preparation"]]$termInfo
	}
	outValue
}

# ------ 2. PREPARATION MEMBER INFORMATION ------
...
#' GBIF Preparation class
#' @seealso \code{\link[DwCTerm]{DwCTerm}}
getGBIFPreparationMembers <- function() {
	outValue <- NULL
	if("http://data.ggbn.org/schemas/ggbn/terms/Preparation" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://data.ggbn.org/schemas/ggbn/terms/Preparation"]]$compositeTerms
	} else if("http://data.ggbn.org/schemas/ggbn/terms/Preparation" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://data.ggbn.org/schemas/ggbn/terms/Preparation"]]$compositeTerms
	}
	outValue
}

# ------ 3. GBIF PREPARATION CLASS ------
#' R6 class representing a data structure for a GBIF Preparation augmented data table (based on the Dawrin core)
#'
#' The \code{GBIFPreparation} class allows for the specification of data tables that comply with the Preparation
#' \url{http://data.ggbn.org/schemas/ggbn/terms/Preparation}{class specification} of GBIF.
GBIFPreparation <- R6Class("GBIFPreparation",
	inherit = DwCGeneric,
	# ====== 3.1. Define private members of the GBIF Preparation class ======
	private = list(
	),
	public = list(
		# ====== 3.2. Define an initialization function for the GBIF Preparation class ======
		#' @description
		#' Create a new GBIFPreparation object
		...
		#' \code{names(getGBIFPreparationMembers())})
		...
		#' for this string.
		#' @param ... A named set of paramaeters corresponding to Darwin core terms associated with the GBIFPreparation
		...
		#' term. Mappable terms can be found using: \code{sapply(X = getGBIFPreparationMembers(), FUN = function(curTerm) { curTerm$getTermName() })}
		#' @return A new \code{GBIFPreparation} object
...
#' @return A new \code{GBIFPreparation} object
#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFPreparationMembers]{getGBIFPreparationMembers}}
initializeGBIFPreparation <- function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
	GBIFPreparation$new(objectData = objectData, idColumnInfo = idColumnInfo, nameAutoMap = nameAutoMap, defDateFormat = defDateFormat, ...)
}
