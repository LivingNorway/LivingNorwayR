# ------ 1. VERNACULARNAME TERM INFORMATION ------
...
getGBIFVernacularNameTerm <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/VernacularName" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/VernacularName"]]$termInfo
	} else if("http://rs.gbif.org/terms/1.0/VernacularName" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/VernacularName"]]$termInfo
	}
	outValue
}
...
#' @seealso \code{\link[DwCTerm]{DwCTerm}}
getGBIFVernacularNameMembers <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/VernacularName" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/VernacularName"]]$compositeTerms
	} else if("http://rs.gbif.org/terms/1.0/VernacularName" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/VernacularName"]]$compositeTerms
	}
	outValue
}
...
GBIFVernacularName <- R6Class("GBIFVernacularName",
	inherit = DwCGeneric,
	# ====== 3.1. Define private members of the GBIF VernacularName class ======
	private = list(
	),
	public = list(
		...
		#' @return A new \code{GBIFVernacularName} object
...
#' @return A new \code{GBIFVernacularName} object
#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFVernacularNameMembers]{getGBIFVernacularNameMembers}}
initializeGBIFVernacularName <- function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
	GBIFVernacularName$new(objectData = objectData, idColumnInfo = idColumnInfo, nameAutoMap = nameAutoMap, defDateFormat = defDateFormat, ...)
}
