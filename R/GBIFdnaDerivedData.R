...
getGBIFdnaDerivedDataTerm <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/dnaDerivedData" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/dnaDerivedData"]]$termInfo
	} else if("http://rs.gbif.org/terms/1.0/dnaDerivedData" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/dnaDerivedData"]]$termInfo
	}
	outValue
}

# ------ 2. DNADERIVEDDATA MEMBER INFORMATION ------
...
getGBIFdnaDerivedDataMembers <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/dnaDerivedData" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/dnaDerivedData"]]$compositeTerms
	} else if("http://rs.gbif.org/terms/1.0/dnaDerivedData" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/dnaDerivedData"]]$compositeTerms
	}
	outValue
}

# ------ 3. GBIF DNADERIVEDDATA CLASS ------
...
GBIFdnaDerivedData <- R6Class("GBIFdnaDerivedData",
	...
	public = list(
		# ====== 3.2. Define an initialization function for the GBIF dnaDerivedData class ======
		#' @description
		#' Create a new GBIFdnaDerivedData object
		...
		#' \code{names(getGBIFdnaDerivedDataMembers())})
		...
		initialize = function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
			super$initialize(
				classTermInfo = getGBIFdnaDerivedDataTerm(),
				associatedTerms = getGBIFdnaDerivedDataMembers(),
