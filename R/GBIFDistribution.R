# ------ 1. DISTRIBUTION TERM INFORMATION ------
...
getGBIFDistributionTerm <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/Distribution" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/Distribution"]]$termInfo
	} else if("http://rs.gbif.org/terms/1.0/Distribution" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/Distribution"]]$termInfo
	}
	outValue
}

# ------ 2. DISTRIBUTION MEMBER INFORMATION ------
...
#' GBIF Distribution class
#' @seealso \code{\link[DwCTerm]{DwCTerm}}
getGBIFDistributionMembers <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/Distribution" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/Distribution"]]$compositeTerms
	} else if("http://rs.gbif.org/terms/1.0/Distribution" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/Distribution"]]$compositeTerms
	}
	outValue
}

# ------ 3. GBIF DISTRIBUTION CLASS ------
#' R6 class representing a data structure for a GBIF Distribution augmented data table (based on the Dawrin core)
#'
#' The \code{GBIFDistribution} class allows for the specification of data tables that comply with the Distribution
#' \url{http://rs.gbif.org/terms/1.0/Distribution}{class specification} of GBIF.
GBIFDistribution <- R6Class("GBIFDistribution",
	inherit = DwCGeneric,
	# ====== 3.1. Define private members of the GBIF Distribution class ======
	private = list(
	),
	public = list(
		# ====== 3.2. Define an initialization function for the GBIF Distribution class ======
		#' @description
		#' Create a new GBIFDistribution object
		...
		#' \code{names(getGBIFDistributionMembers())})
		...
		#' for this string.
		#' @param ... A named set of paramaeters corresponding to Darwin core terms associated with the GBIFDistribution
		...
		#' term. Mappable terms can be found using: \code{sapply(X = getGBIFDistributionMembers(), FUN = function(curTerm) { curTerm$getTermName() })}
		#' @return A new \code{GBIFDistribution} object
...
#' @return A new \code{GBIFDistribution} object
#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFDistributionMembers]{getGBIFDistributionMembers}}
initializeGBIFDistribution <- function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
	GBIFDistribution$new(objectData = objectData, idColumnInfo = idColumnInfo, nameAutoMap = nameAutoMap, defDateFormat = defDateFormat, ...)
}
