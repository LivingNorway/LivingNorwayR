...
getGBIFAmplificationTerm <- function() {
	...
	if("http://data.ggbn.org/schemas/ggbn/terms/Amplification" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://data.ggbn.org/schemas/ggbn/terms/Amplification"]]$termInfo
	} else if("http://data.ggbn.org/schemas/ggbn/terms/Amplification" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://data.ggbn.org/schemas/ggbn/terms/Amplification"]]$termInfo
	}
	outValue
}

# ------ 2. AMPLIFICATION MEMBER INFORMATION ------
...
getGBIFAmplificationMembers <- function() {
	outValue <- NULL
	if("http://data.ggbn.org/schemas/ggbn/terms/Amplification" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://data.ggbn.org/schemas/ggbn/terms/Amplification"]]$compositeTerms
	} else if("http://data.ggbn.org/schemas/ggbn/terms/Amplification" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://data.ggbn.org/schemas/ggbn/terms/Amplification"]]$compositeTerms
	}
	outValue
}

# ------ 3. GBIF AMPLIFICATION CLASS ------
#' R6 class representing a data structure for a GBIF Amplification augmented data table (based on the Dawrin core)
#'
#' The \code{GBIFAmplification} class allows for the specification of data tables that comply with the Amplification
#' \url{http://data.ggbn.org/schemas/ggbn/terms/Amplification}{class specification} of GBIF.
GBIFAmplification <- R6Class("GBIFAmplification",
	inherit = DwCGeneric,
	# ====== 3.1. Define private members of the GBIF Amplification class ======
	private = list(
	),
	public = list(
		# ====== 3.2. Define an initialization function for the GBIF Amplification class ======
		#' @description
		#' Create a new GBIFAmplification object
		...
		#' \code{names(getGBIFAmplificationMembers())})
		...
		#' for this string.
		#' @param ... A named set of paramaeters corresponding to Darwin core terms associated with the GBIFAmplification
		...
		#' term. Mappable terms can be found using: \code{sapply(X = getGBIFAmplificationMembers(), FUN = function(curTerm) { curTerm$getTermName() })}
		#' @return A new \code{GBIFAmplification} object
...
#' @return A new \code{GBIFAmplification} object
#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFAmplificationMembers]{getGBIFAmplificationMembers}}
initializeGBIFAmplification <- function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
	GBIFAmplification$new(objectData = objectData, idColumnInfo = idColumnInfo, nameAutoMap = nameAutoMap, defDateFormat = defDateFormat, ...)
}
