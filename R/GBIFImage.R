...
getGBIFImageTerm <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/Image" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/Image"]]$termInfo
	} else if("http://rs.gbif.org/terms/1.0/Image" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/Image"]]$termInfo
	}
	outValue
}

# ------ 2. IMAGE MEMBER INFORMATION ------
...
#' GBIF Image class
#' @seealso \code{\link[DwCTerm]{DwCTerm}}
getGBIFImageMembers <- function() {
	outValue <- NULL
	if("http://rs.gbif.org/terms/1.0/Image" %in% names(GBIFCoreClassList)) {
		outValue <- GBIFCoreClassList[["http://rs.gbif.org/terms/1.0/Image"]]$compositeTerms
	} else if("http://rs.gbif.org/terms/1.0/Image" %in% names(GBIFExtClassList)) {
		outValue <- GBIFExtClassList[["http://rs.gbif.org/terms/1.0/Image"]]$compositeTerms
	}
	outValue
}
...
#'
#' The \code{GBIFImage} class allows for the specification of data tables that comply with the Image
#' \url{http://rs.gbif.org/terms/1.0/Image}{class specification} of GBIF.
GBIFImage <- R6Class("GBIFImage",
	inherit = DwCGeneric,
	# ====== 3.1. Define private members of the GBIF Image class ======
	private = list(
	),
	public = list(
		# ====== 3.2. Define an initialization function for the GBIF Image class ======
		#' @description
		...
		#' \code{names(getGBIFImageMembers())})
		...
		#' for this string.
		#' @param ... A named set of paramaeters corresponding to Darwin core terms associated with the GBIFImage
		...
		#' term. Mappable terms can be found using: \code{sapply(X = getGBIFImageMembers(), FUN = function(curTerm) { curTerm$getTermName() })}
		#' @return A new \code{GBIFImage} object
		#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFImageMembers]{getGBIFImageMembers}}
		initialize = function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
			super$initialize(
				classTermInfo = getGBIFImageTerm(),
				associatedTerms = getGBIFImageMembers(),
...
#' @return A new \code{GBIFImage} object
#' @seealso \code{\link[DwCTerm]{DwCTerm}} \code{\link[getGBIFImageMembers]{getGBIFImageMembers}}
initializeGBIFImage <- function(objectData, idColumnInfo, nameAutoMap = FALSE, defDateFormat = "YYYY-MM-DD", ...) {
	GBIFImage$new(objectData = objectData, idColumnInfo = idColumnInfo, nameAutoMap = nameAutoMap, defDateFormat = defDateFormat, ...)
}
